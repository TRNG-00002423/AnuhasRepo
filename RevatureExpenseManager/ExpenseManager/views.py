...
def viewExpenses(request, id):
    ...
    # approvals = Expense.objects.select_related('expense_id').all()
    # approvals_filter = Approval.objects.filter(expense_id=1)
    # x = 0
    # for item in expenses:
    #     approvals_filter[x] = Approval.objects.filter(expense_id=item.pk).all()
    #     x = x+1
    # list_of_expenseIds = list()
    # approvals = Approval.objects.select_related('reviewer').all()
    # for expense in expenses:
    #     list_of_expenseIds.add(expenses.pk)
    # for item in list_of_expenseIds:
    #     approvals.add(Approval.objects.select_related('reviewer').filter(expense_id=item))
    # approvals = Approval.objects.select_related('').filter(user_id=users.email)
    # return render(request, 'EmployeeExpenses.html', {'expenses': expenses, 'approvals' : approvals})
