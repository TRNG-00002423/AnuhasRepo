...
by_module_duration = sorted(test_results, key= lambda x: (x["duration_ms"], x["module"]))
# by_module_duration = sorted(by_duration, key= lambda x: x["module"])


# print(" ")
# print(f"{by_module_duration}")
# print(" ")
...
names = list(map(lambda x: x['name'], test_results))  # TODO — use map()

# # TODO 2b: Transform each record into a status badge string.
# # Format: "✅ test_login (1200ms)"  or  "❌ test_search (850ms)"
badges = list(map(lambda x: str(x), test_results))  # TODO — use map() with a lambda

# # TODO 2c: Extract the set of unique module names.
# # Expected: {'auth', 'search', 'checkout', 'profile'}
modules = set(map(lambda x: x["module"], test_results))  # TODO — use set() + map()
...
failures = list(filter(lambda x: x["status"]=="fail", test_results))  # TODO — use filter()

# # TODO 3b: Get all tests slower than 1500ms.
slow_tests = list(filter(lambda x: x["duration_ms"] > 1500, test_results))  # TODO — use filter()
...
critical = list(filter(lambda x: x["duration_ms"] > 1500, (filter(lambda x: x["status"]=="fail", test_results))))  # TODO
...
# # Expected: 15560
total_duration = reduce(lambda acc, x: acc + x["duration_ms"], test_results, 0)  # TODO — use reduce() with initializer=0

# # TODO 4b: Total duration of FAILED tests only.
# # # Hint: chain filter() before reduce().
total_fail_duration = reduce(lambda acc, x: acc + x["duration_ms"], (filter(lambda x: x["status"]=="fail", test_results)),0)   # TODO

# # TODO 4c: Find the name of the test with the LONGEST name (by character count).
longest_name = reduce(lambda longest, name: name if len(name)>len(longest) else longest, names)  # TODO — reduce() over names
...
module_counts = reduce(lambda acc, x: {**acc, x["module"]: acc.get(x["module"], 0) + 1}, test_results, {})  # TODO — reduce() with a dict accumulator and initializer={}
...
test_names, test_modules, test_durations, test_statuses = zip(*[[r['name'], r['module'], r['duration_ms'], r['status']] for r in test_results])

# # TODO 5c: Build a dict mapping test names to duration_ms using zip().
# # Expected: {'test_login': 1200, 'test_register': 2100, ...}
# name_to_duration = dict(zip(map(lambda x: x["name"], test_results),(map(lambda x: x["duration_ms"], test_results),)))  # TODO
name_to_duration = dict(zip(test_names, test_durations))  # TODO
...
avg_fail_duration = (reduce(lambda acc, r: acc + r["duration_ms"], filter(lambda x: x["status"]=="fail", test_results),0,)/ len(list(filter(lambda x: x["status"]=="fail", test_results))))
